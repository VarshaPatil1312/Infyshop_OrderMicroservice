...
public class ProductsOrderedController {
	...
	public ResponseEntity<String> addProduct(@RequestBody ProductsOrderedDTO productsOrderedDTO){
		SubscribedproductDTO[] subscribedproductsDTO = new RestTemplate().getForObject("http://localhost:8100/api/subscriptions/"+productsOrderedDTO.getBuyerId(), SubscribedproductDTO[].class);
