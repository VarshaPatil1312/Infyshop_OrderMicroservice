...
public class OrderController {
	...
	public ResponseEntity<String> addProductsToCart(@RequestBody CartDTO cartDTO){
		SubscribedproductDTO[] subscribedproductsDTO = new RestTemplate().getForObject("http://localhost:8100/api/subscriptions/"+cartDTO.getBuyerId(), SubscribedproductDTO[].class);
	...
	public ResponseEntity<String> deleteProductsToCart(@RequestBody CartDTO cartDTO){
		String message = new RestTemplate().postForObject("http://localhost:8300/cart/delete", cartDTO, String.class);
		return new ResponseEntity<>(message,HttpStatus.OK);
	}
}
